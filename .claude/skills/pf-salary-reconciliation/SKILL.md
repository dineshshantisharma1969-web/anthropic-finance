...
- `ESIC_NEW_vs_FUTURE = ROUND(ESIC_NEW − ESIC AS PER FUTURE, 2)` — the gap; **0 = consistent**, `>0` = ESI
  looks missing (worker appears ESI-eligible yet Future shows less). `REVISED_ESIC = ESIC AS PER FUTURE`
  stays the anchor and is never changed — this column only surfaces where the tools need a nudge.
All four are written as **live Excel formulas** (`write_esi_formulas`): `REVISED_GROSS_NEW = MAX(0, ESI WAGES
− WASHING ALLOWANCE)`, `ESIC_NEW = ROUND(0.0075×REVISED_GROSS_NEW,2)`, `ESIC_NEW_vs_FUTURE = ROUND(ESIC_NEW
− ESIC AS PER FUTURE,2)`, `PROJECTED_GROSS_NEW = ROUND(MIN(…),0)` — so editing the tools (days / ESI WAGES /
washing) recomputes them in-sheet.
