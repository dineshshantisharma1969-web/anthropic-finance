...
| # | Item | Status |
|---|---|---|
| 1 | **April-26 excess salary — TRIAGED & PATCHED (Rule M16, 2026-07-04)**: headline ₹5.45 Cr was inflated by projection artifacts; after M16 (cap excess at gross + suppress ≤2-day flags — `april-2026/SKILL_PATCH_M16_excess_artifact.md` + `patch_excess_artifact_M16.py`) the real flagged excess is **₹1.91 Cr on 4,775 rows** (golden rules verified intact). Remaining human decisions packaged as **Rule M17 worklist** (`April26_WORKLIST_Recovery_ESI.xlsx`, generator `make_worklist_M17.py` + `SKILL_PATCH_M17_worklist.md`): 🔴 RECOVERY_REVIEW tab (M18: expected = FIXEDGROSS/SITEDIVISIONDAYS × NORMALDAYS **+ allowances** (OT/arrears/attendance/holiday); cleared if within max(₹500,2%) of gross/revised gross — ALL 297 per-day-rate rows cleared, 588 false rows total, ₹14.85L phantom removed) — **130 HIGH rows > ₹10k = ₹15.9L** (+2,059 small rows ₹66.4L) with VERIFIED/DECISION working columns; 🟠 ESI_ENROLLMENT tab — 1,940 employees by client, ₹15.4L/month exposure, with IC-number/enrolled-from columns. Patched file: `April26_RECONCILED_FINAL_M16_PATCHED.xlsx` (delivered 2026-07-04). Apply M16→M17 to all future months. | 🟠 **OPEN — team to work the M17 worklist** |
