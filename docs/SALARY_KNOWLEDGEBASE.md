...
| # | Item | Status |
|---|---|---|
| 1 | **April-26 excess salary — TRIAGED & PATCHED (Rule M16, 2026-07-04)**: headline ₹5.45 Cr was inflated by projection artifacts; after M16 (cap excess at gross + suppress ≤2-day flags — `april-2026/SKILL_PATCH_M16_excess_artifact.md` + `patch_excess_artifact_M16.py`) the real flagged excess is **₹1.91 Cr on 4,775 rows** (golden rules verified intact). Remaining human decisions packaged as **Rule M17 worklist** (`April26_WORKLIST_Recovery_ESI.xlsx`, generator `make_worklist_M17.py` + `SKILL_PATCH_M17_worklist.md`): 🔴 RECOVERY_REVIEW tab (M18a-c: rate basis auto-detected — div=1 OR FIXEDGROSS<₹3,000 → PER-DAY (the divisor column lies on 58 rows); expected = rate × **NORMALDAYS** + positive allowances only; cleared if within max(₹500,2%) of gross/revised gross — 644 false rows cleared, ₹20.2L phantom removed) — **100 HIGH rows > ₹10k = ₹11.3L** (+2,033 small rows ₹64.8L) with VERIFIED/DECISION working columns; 🟠 ESI_ENROLLMENT tab — 1,940 employees by client, ₹15.4L/month exposure, with IC-number/enrolled-from columns. Patched file: `April26_RECONCILED_FINAL_M16_PATCHED.xlsx` (delivered 2026-07-04). Apply M16→M17 to all future months. | 🟠 **OPEN — team to work the M17 worklist** |
| 1b | **Wage Code 50% test (added to M17 worklist 2026-07-04):** 1,050 April-26 rows fail `BASIC+DA ≥ 50% of CTC` (1,019 structural, 46 severe <30%; 234 negative-gross reversals excluded; monthly shortfall to 50% ≈ ₹25.5L). Concentrated: Ministry of Railways, Blink Commerce 58%, Bhagwati Products 87%, JLL, Zomato, Infosys. Tab `WAGE_CODE_50PCT` in the worklist; restructuring raises PF/gratuity cost — price before deciding | 🟠 OPEN — restructure decision |
