...
| 2026-07-10 | Corrections DEPLOYED | 15 updated | Both fixes applied to the LIVE loop workflow `hqrLbu8oBQxIURU3` (patched jsCode incl. Txn Key block, republished). 15 sheet rows corrected via one-off workflow `nAdoD8cpRDRuObsj`; Dashboard + Client Receipts refreshed and verified: cumulative net = −₹24.71 cr. `scripts/ispl_bank_statement_loop.workflow.ts` synced with the live jsCode. |
| 2026-07-10 | Dashboard interest columns | — | **ISPL Bank Dashboard Refresh** (`tqYOP2Rvgc9kI43F`) now writes two extra columns: `Interest @7.9% (Day)` = WCDL daily interest on that day's cumulative deficit (0 on surplus days, rate 7.9%/365, simple), and `Cumulative Interest` = month-to-date total. Clear range widened A2:G → A2:I. Verified: July 1–10 cumulative interest ₹2,40,136. To change the rate, edit `WCDL_RATE` in the workflow's Code node. |
