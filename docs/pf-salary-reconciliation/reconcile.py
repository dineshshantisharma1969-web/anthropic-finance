...
def reconcile(sal, ecr, fut, fmd):
    ...
    # ORIGINAL ESI actually deducted in the salary sheet (the true "is this employee
    # in ESI?" signal — NOT Future-sheet membership). Drives the corrected ESI
    # coverage-gap flag below (see SKILL_PATCH_esi_coverage_flag).
    oesic_col = resolve(sal, "ESIC", required=False)
    sal["OESIC_v"] = num(sal[oesic_col]) if oesic_col else pd.Series(0.0, index=sal.index)
